...
class DeleteUserList:
    ...
    def execute(self, command: DeleteUserListCommand) -> None:
        try:
            id = UserListId(command.id)
        except Exception as e:
            raise InvalidUserListDataError(
                f"Invalid user list id for delete: {str(e)}"
            ) from e
        ...
        self.user_list_repository.delete(user_list)
